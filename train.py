...
from util.torch_utils import smart_optimizer
import torch
...
def get_args_parser():
    ...
    parser.add_argument('--smart_opt', action='store_true')
    # Model parameters
...
def main(args):
    ...
    optimizer = torch.optim.Adam(param_dicts, lr=args.lr * 0.1, weight_decay=args.weight_decay)
    lr_scheduler = CosineAnnealingWarmUpRestarts(optimizer, T_0=args.epochs, T_mult=1, eta_max=args.lr, T_up=args.warmup)
    #optimizer = torch.optim.SGD(param_dicts, lr=args.lr * 0.1)
    #optimizer = smart_optimizer(model, 'Adam', lr = args.lr, weight_decay=args.weight_decay)
    #optimizer = torch.optim.Adam(param_dicts, lr=args.lr * 0.1)
