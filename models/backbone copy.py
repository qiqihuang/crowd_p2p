...
class BackboneBase_VGG(nn.Module):
    def __init__(self, backbone: nn.Module, num_channels: int, name: str, return_interm_layers: bool = True):
        ...
        if return_interm_layers:
            self.body1 = nn.Sequential(*features[:13]) # 2x down
            self.body2 = nn.Sequential(*features[13:23]) # 4x down
            self.body3 = nn.Sequential(*features[23:33]) # 8x down
            self.body4 = nn.Sequential(*features[33:43]) # 16x down
            # self.body4 = nn.Sequential(*features[33:44]) # 32x down

    def forward(self, x):
        C2 = self.body2(self.body1(x))
        C3 = self.body3(C2)
        C4 = self.body4(C3)
        return C2, C3, C4
